...
tool-pi-game の current release-driven development で使用する Operating Model を定義します。
...
- GitHub Issues を durable implementation / dependency SoT とする
- host repository work stateをcanonical copyしない
- external planning surface は明示的に採用された場合だけ canonical とする
...
- The repository's current default branch is `develop`; this governance PR intentionally targets that branch rather than assuming `main`.
- Source ownership stays in this repository; host integration evidence stays bound to the my-web-2025 candidate/layout.
- Bun in mise is the project-init Skills CLI prerequisite; it does not silently replace host/application package-manager authority.

## Agent Skills lifecycle

- current `rebuildup/project-init` via `bunx skills`.
- canonical layout: `.agents/skills/` + `.claude/skills/`.
- bootstrap: `mise run skills-bootstrap`; update: `mise run skills-update` / `bunx skills update -p -y`.
- `skills-lock.json` is durable source/content metadata; `--global` is not freshness evidence.
